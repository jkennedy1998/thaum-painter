...
## purpose
Own direct brush-style cell application and the shared blank-cell write seam.

## owns
- single-cell draw semantics
- blank-cell writes that remove the target cell
- brush-to-cell mapping rules including glyph/sprite graphic, portable flat/material/slot color, weight, and ordered shader-stack appearance payloads
...
- `brush.rs` inline `#[cfg(test)]` module
  - footprint size/shape in the default view, depth axis fixed at every swing (PosX: x never moves, z/y spread), and clear-character writes removing only their target cell
