...
fn collect_canvas_patches(before: &Canvas, after: &Canvas) -> Vec<SharedCellPatch> {
    ...
    positions
        ...
        .filter_map(|position| {
            ...
            if b == a { None } else { Some(SharedCellPatch::new(position, b, a)) }
        })
        .collect()
}
...
fn paint_flow_end_to_end_on_fresh_document() {
    ...
    let bounds = CanvasBounds {
        x0: -10, y0: -8, x1: 10, y1: 8, z: 0, plane_axis: CanvasPlaneAxis::Z,
    };
    ...
    println!("left target: {:?}", tool_state.hand_state(PaintHand::Left).target);
    assert_eq!(tool_state.hand_state(PaintHand::Left).target, PaintTarget::Image);
    ...
    tool_state.apply_at_for_hand(&mut candidate, &mut selection, CellPoint { x: 0, y: 0, z: 0 }, PaintHand::Left, bounds, flat_view());
    ...
    let canvas_view = runtime.canvas_for_layer("layer-1", 0).expect("canvas_for_layer after stage");
    println!("runtime canvas cells after stage: {}", canvas_view.len());
    assert!(canvas_view.contains_key(&CellPoint { x: 0, y: 0, z: 0 }), "staged cell invisible in render path");
    ...
    let record = SharedDocumentActionRecord::cell_patch_set(
        "1", "doc-1", "layer-1", "tester", "2026-09-02T00:00:00Z", patches, Some(block_id.clone()),
    );
    ...
    if let Some(revert) = runtime.undo_top_action("layer-1") {
        let rec = SharedDocumentActionRecord::revert_patch_set(
            "2", "doc-1", "layer-1", "tester", "2026-09-02T00:00:01Z", revert.patches,
            Some(revert.block_id), revert.action_id,
        );
        append_action_record(&paths.actions_file_path, &rec).unwrap();
        runtime.push_history_record(rec);
    }
    let canvas_view = runtime.canvas_for_layer("layer-1", 0).unwrap();
    assert!(!canvas_view.contains_key(&CellPoint { x: 0, y: 0, z: 0 }), "undo did not revert cell");
}
...
fn paint_flow_on_real_local_document() {
    ...
    if !root.exists() { panic!("local-document not found at {:?}", root); }
    let paths = SharedDocumentPaths::new(root.to_path_buf());
    let document = thaum_painter_domain::storage::load_document_file(&paths.document_file_path).unwrap();
    let actions = thaum_painter_domain::storage::load_action_records(&paths.actions_file_path).unwrap();
    ...
    let breath = runtime.first_breath_with_raster_block("layer-1").expect("raster track exists");
    ...
    let bounds = CanvasBounds {
        x0: -60, y0: -50, x1: -40, y1: -30, z: -12, plane_axis: CanvasPlaneAxis::Z,
    };
    let mut selection = PainterSelection::new(bounds);
    let mut tool_state = ToolState::default();
    tool_state.set_tool_for_hand(PaintHand::Left, PaintTool::Brush);
    ...
    let before = canvas.clone();
    let mut candidate = before.clone();
    tool_state.apply_at_for_hand(&mut candidate, &mut selection, CellPoint { x: -50, y: -40, z: -12 }, PaintHand::Left, bounds, flat_view());
    let patches = collect_canvas_patches(&before, &candidate);
    println!("press patches (no selection): {}", patches.len());
    assert!(!patches.is_empty(), "brush press produced no patches with empty selection");
    runtime.stage_canvas_patches("layer-1", &block_id, &patches);
    canvas = candidate;

    let view = runtime.canvas_for_layer("layer-1", breath).expect("canvas after stage");
    assert!(view.contains_key(&CellPoint { x: -50, y: -40, z: -12 }), "staged cell invisible");
    ...
    let mut selection = PainterSelection::new(bounds);
    selection.apply_plane_points([CellPoint { x: -60, y: -50, z: -12 }]);
    let before = canvas.clone();
    let mut candidate = before.clone();
    tool_state.apply_at_for_hand(&mut candidate, &mut selection, CellPoint { x: -55, y: -45, z: -12 }, PaintHand::Left, bounds, flat_view());
    let patches = collect_canvas_patches(&before, &candidate);
