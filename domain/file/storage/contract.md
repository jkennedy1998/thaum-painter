...
- `storage.rs`'s live `SharedDocumentLayer`/`SharedDocumentRuntime` runtime schema (used by the running app) carries `visible`/`locked`/fixed `origin`/`start_breath`/`length_breaths` per layer, with `set_layer_visible`, `set_layer_locked`, `set_layer_origin`, `rename_layer`, `remove_layer`, and `set_layer_timing` on the runtime. Fixed origin is serialized separately from animated move-track offsets; `composited_canvas_in_layer_order` skips invisible layers. Every persisted patch/canvas cell now retains glyph-or-sprite graphic, flat/material/A-B-C-slot color values, weight, and its ordered filename shader stack. `start_breath`/`length_breaths` are purely an authoring/UI concept surfaced by `domain/modules/individuals/layers-panel/` for now — they do not gate compositing. This is still a simpler live/collaborative schema than the canonical save schema in `domain/file/file-schema/` (which also carries `opacity`, `placement`, full `timing`, and per-property blocks) — closing that structural gap is future work.
...
- Host-guard exemption (settled J 2026-09-09): while the runtime hosts a live session (`session_hosting`), both guards are off — foreign joiner records reach the host through sync, never through this disk's append path, so the runtime log legitimately outgrows the solo log and the guards would false-fire forever (the reload they triggered also tried to read a document.json that was never written — the doc-reload error spam). The host IS the one disk writer while it hosts; host leave force-saves the full session truth so solo guarded saves resume clean.
