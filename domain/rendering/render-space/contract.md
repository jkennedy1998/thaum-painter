...
- binary-bars update (2026-09-07): the two `resolve_gap_fill` call sites collapsed — a breath not covered by a solid block/segment is an **empty** and renders nothing (interim stub approved by J). The `interpolation/` dependency is gone (encapsulation deleted; per-row custom interpolation — move lerp vs raster, user-settable empty mode — is a later pass outside the bars project, see `context/bars-binary-design-truth.md`).
