...
- stamp seams: `stamp_changes_for_hand` places the hand user's copied world cells at an anchor with the copied cells' own graphic/color/weight (enabled edit channels paste the copied value, locked channels keep the target cell's existing value, selection-gated, gfx-locked stamps skipping empty target cells entirely under the unified empty-cell rule), and `stamp_preview_cells` pairs current cells with the upcoming stamp for the two-phase flash
