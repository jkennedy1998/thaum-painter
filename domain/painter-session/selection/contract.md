...
- source-of-truth from J (void marker): in the selection flash's true-appearance phase, an empty cell (absent or authored blank) displays as a vivid `●` glyph at weight 0 — not a placeholder box — so selected voids read as voids. The vivid-recolor phase keeps its existing empty-cell marker.
