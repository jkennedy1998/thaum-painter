...
- tool-property matrix columns enable per hand by what THAT hand's equipped tool declares (J 2026-09-12): the picker declares `brush_size` because it samples through the hand's brush-tip footprint, so its size tokens stay clickable while the picker is up — the old hard-gate to the Brush tool rendered the picker's size tokens dead, so size could only be preset before switching tools.
