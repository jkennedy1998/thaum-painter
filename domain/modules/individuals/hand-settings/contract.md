...
## purpose
Own painter's compact hand-settings panel: the left/right paint-state editor for weight, edit/select masks, target mode, selection mode, tool-property rows, and channel locks.

## owns
- the `HandSettingsModule` type
- the compact per-hand settings layout and hit-testing
- the two-zone row layout: always-shown standard hand rows (weight, select/edit masks, locks, target, selection mode) on top, tool-specific rows below that only render when either equipped hand's tool declares them via `tool-state`'s per-tool property manifest
...
- inline `#[cfg(test)]` in `hand_settings_module.rs`
  - light
  - validates weight, select/edit masks, target, selection mode, brush/fill property, and lock toggles, plus tool-row hiding driven by the equipped tools

## data
- none

## notes
- source-of-truth from J: left and right should each keep their own color/weight selection, with useful masking and lock controls for deeper illustration work.
