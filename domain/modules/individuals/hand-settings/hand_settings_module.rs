...
impl Module for HandSettingsModule {
    ...
    /// Tooltip hotspots: the module's gizmo bar plus one hotspot per visible
    /// property row, generated by the shared property-rows seam.
    fn hotspots(&self) -> Vec<Hotspot> {
        let custom = PropertyRows::hotspots(self.rect, &self.visible_rows());
