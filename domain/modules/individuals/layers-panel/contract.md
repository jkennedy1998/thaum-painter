...
- inline layer rename via the shared `thaum-renderer` text-entry seam — double-click the name to open a `TextEntryField` draft on the registry's key-capture hook (J 2026-09-12); drag-based reorder is still deliberately deferred
...
- this module consumes two newer shared `thaum-renderer` seams rather than re-rolling them (J 2026-09-12):
  - `shared/text-cells` `push_text_cells` — every glyph run in `draw()` (names, ruler labels, transport buttons) goes through the one helper with an explicit `CellWeight`; names ride the char-button weight band (weight 1 at rest, weight 2 hovered/rename-live/selected property).
  - `shared/click-timing` `DoubleClick` — the two former hand-rolled `Recent…Click` structs are now one `DoubleClick` per surface: property blocks keyed on the `(button, layer, property, block, piece, cell_type)` subject, layer names keyed on the layer id; the canonical `DOUBLE_CLICK_WINDOW` (350 ms) is seam-owned.
- the eye/lock/delete buttons consume the shared `char_button_cell` seam (bright weight 1 at rest, vivid weight 2 on hover); interactive text buttons (AUTO KEY, PLAY, LOOP, + NEW LAYER) render at weight 1 through the same band, and the timeline breath labels (start/current/end, loop-window edges) also read at weight 1 — no timeline number sits at the invisible weight 0 (J 2026-09-12).
