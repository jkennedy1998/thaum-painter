...
- inline `#[cfg(test)]` in `layers_panel_module.rs`
  ...
  - double-click branches: double-left on a solid single or center queues `DuplicatePropertyBlock` (non-destructive right-landing push); double-right on a solid single or center queues `BlankPropertyBlock`; double-left on a solid left/right head merges the adjacent bar in as one `SetPropertyBlockTimingDestructive` (rejected at the span edge, mirrored for right heads); double-right on any empty queues `MergeEmptyPropertyBlock` (left-preferred seam, right heads mirror; rejected on a fully-empty track at the storage seam); double-left on empties and double-right on solid heads are unused
