...
## purpose
Own the two build commands for thaum-painter — the human-facing way to build and run the app — plus the entrypoint binary crate they build. One build pipeline, shared by both commands, producing replaceable per-environment lane folders.
...
- `build-all-environments` builds lanes and never runs them. Default: every known environment; pass names to build a subset.
...
- desktop launcher entries for both commands (`*.desktop`)
...
- GitHub CI release builds (future workflow; will reuse this lane layout; mac lane needs it)
...
- `run-and-build-linux.desktop`, `build-all-environments.desktop` — double-click launchers
