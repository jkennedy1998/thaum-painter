...
- Source-of-truth from J: regular SAVE must never crash or hang on a dialog. Quick save writes `artifacts/painter-files/<slugified-title>/` with `-NN` bump, never prompts; native dialog stack only on SAVE AS/OPEN (XDG portal backend failed instantly on jobo). Save root moved from `context/painter/painter-files/` to `artifacts/painter-files/` (J 2026-09-10: runtime output lives under `artifacts/`).
