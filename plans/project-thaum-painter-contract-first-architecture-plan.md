...
- `thaum-painter/domain/rendering/`: edit
- `thaum-painter/domain/painter-document/`: edit
- `thaum-painter/domain/painter-document/modules/`: add
- `thaum-painter/domain/painter-session/`: edit
- `thaum-painter/domain/painter-operations/`: edit
- `thaum-painter/context/`: edit

## encapsulation-plans
- `thaum-painter/domain/file/`: `plans/home-j-repos-thaum-painter-domain-file-plan.md`
- `thaum-painter/domain/rendering/render-space/`: `plans/home-j-repos-thaum-painter-domain-rendering-render-space-plan.md`
- `thaum-painter/domain/painter-document/`: `plans/home-j-repos-thaum-painter-domain-painter-document-plan.md`
- `thaum-painter/domain/painter-session/`: `plans/home-j-repos-thaum-painter-domain-painter-session-plan.md`
- `thaum-painter/domain/painter-operations/`: `plans/home-j-repos-thaum-painter-domain-painter-operations-plan.md`
...
- 26-08-31: the 26-08-29 module concept is reversed — see `context/module-concept-audit.md`'s "superseded" section for full reasoning (short version: the renderer's camera turned out to be universal, not module-local, so the reason to bundle several layers into one `CellGroup` before handoff no longer applies). `domain/painter-document/modules/` is removed; `domain/painter-document/groups/` absorbs its duties directly. `document.modules[*].groups[]` collapses back to a flat `document.groups[]`, one group per renderer `CellGroup`. `domain/rendering/render-space/`, `domain/file/manifest/`, `domain/painter-document/groups/`, `context/roadmap.md`, and the four audit/matrix/review docs were all updated with pointer notes; `manifest.rs`/`render_space.rs`/the v1 example+schema JSON still need a matching code pass. Also settled in this pass: a new `domain/modules/` encapsulation (both here and in `thaum-renderer`) will own the renderer's interactive UI-panel format — separate design thread, not yet contracted.
