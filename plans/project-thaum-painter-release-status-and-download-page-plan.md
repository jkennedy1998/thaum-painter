...
- consumers: Thaum Painter supplies the release-status button; existing buttons retain their current default appearance and no tooltip unless configured.
- artifacts: none
...
- consumers: the Painter frame loop renders the button; users click it to visit the page; `release-thaum-painter` publishes the new site source only after GitHub Release asset verification.
...
- action: new
- intent: Create the only public Thaum Painter download/update destination in the existing portfolio-page visual system.
- interface delta: static `/thaum-painter/` route and `release.json` manifest with the stable release version and canonical page URL.
...
- [ ] confirm its one source entry contains the product summary, current version, the three existing platform download links, and concise platform instructions.
...
- [ ] confirm generic button presentation and tooltip metadata do not alter existing buttons unless opted in.
...
- [ ] confirm `release-thaum-painter` validates version alignment, verifies the GitHub Release assets, then updates/pushes the website’s page/manifest source and generated output.
